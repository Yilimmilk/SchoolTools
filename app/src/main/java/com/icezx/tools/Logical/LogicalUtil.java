...
// private final static String LOWESTSING = "#";
    ...
    public LogicalUtil(String expBase) {
        init(expBase,this.precision,this.roundingMode);
    }
    public LogicalUtil(String expBase,int precision,RoundingMode roundingMode){
...
//  stack.push(LOWESTSING);
    ...
    public static boolean compareTo(String strParm){
        ...
        if(-1 != str.indexOf(">=")){
            ...
            int r = new BigDecimal((new LogicalUtil(strTemps[0]).caculate())).compareTo(new BigDecimal((new LogicalUtil(strTemps[1]).caculate())));
            if( -1 == r ){
                reBoolean = false;
            }else{
                reBoolean = true;
...
//  System.out.println(resultStr);
        LogicalUtil me = new LogicalUtil("-(-1.5000000003+0.1)*10+2");
...
//  System.out.println(m.matches());
        boolean reBoolean = LogicalUtil.compareTo("((-8.0+3)>=2. 1)");
